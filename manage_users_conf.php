        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if ($row['add_card'] == 0) {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    $sql = "SELECT vehiclenumber FROM users WHERE vehiclenumber=? AND id NOT like ?";
                    $result = mysqli_stmt_init($conn);
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        echo "SQL_Error";
                        exit();
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            $sql="UPDATE users SET username=?, vehiclenumber=?, gender=?, email=?, user_date=CURDATE(), device_uid=?, device_dep=?, add_card=1 WHERE id=?";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            echo "The vehicle number is already taken!";
                            exit();
                        }
                    }
                }
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email)) {
                                ...
                                $sql="UPDATE users SET username=?, vehiclenumber=?, gender=?, email=?, device_uid=?, device_dep=? WHERE id=?";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
